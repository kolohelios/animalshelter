...
describe('GET /api/pets/dogs/{size}', function(){
  ...
  it('should return medium dogs', function(done){
    server.inject({method: 'GET', url: '/api/pets/dogs/medium'}, function(response){
      expect(response.statusCode).to.equal(200);
      expect(response.result.length).to.equal(1);
      done();
    });
  });
  it('should get no records returned', function(done){
    server.inject({method: 'GET', url: '/api/pets/dogs/giant'}, function(response){
      expect(response.statusCode).to.equal(200);
